...
import { SAMPLING_FREQUENCY, FREQ_RATIO_SQRT, FREQ_RATIO, getOvertoneFrequencies } from './constants';

function gen_timbre_samples(o: { fundamentalFrequency: number; lengthInSecond: number; powerSpectrum: (k: number) => number; }): number[] {
...
write_sound_to_file("sawtooth_equivalent.wav", gen_timbre_samples({ fundamentalFrequency: PITCH_STANDARD, lengthInSecond: 1, powerSpectrum: (k: number) => 1 / k }));
