...
class Timbre {
    ...
    gen_samples(o) {
        ...
        const samples = Array.from({ length: Math.ceil(sample_length_in_second * constants_1.SAMPLING_FREQUENCY) }, (_, i) => {
            const second = i / constants_1.SAMPLING_FREQUENCY;
            // 基本周波数の k 倍の音は振幅が powerSpectrum(k) 倍になる
            return (0, constants_1.getOvertoneFrequencies)(fundamental_frequency)
                .map((freq, i) => Math.sin(second * freq * 2 * Math.PI) * this.powerSpectrum(constants_1.FREQ_RATIO[i]))
...
const sawToothTimbre = new Timbre((k) => 1 / k);
function write_sound_to_file(outPath, samples) {
    let wav = new wavefile.WaveFile();
    wav.fromScratch(1, 44100, '32', samples);
    fs_1.default.writeFileSync(outPath, wav.toBuffer());
}
const PITCH_STANDARD = 443.0959607292108;
const SEMITONE_RATIO = Math.pow(constants_1.FREQ_RATIO[1], 1 / 18); // 1.0579488485113508
...
function genChord(semitones_arr, outPath) {
    ...
    const samples_arr = semitones_arr.map((semitones) => sawToothTimbre.gen_samples({
        fundamentalFrequency: PITCH_STANDARD * Math.pow(SEMITONE_RATIO, semitones),
        lengthInSecond,
    }));
    write_sound_to_file(outPath, Array.from({ length: lengthInSecond * constants_1.SAMPLING_FREQUENCY }, (v, i) => semitones_arr.map((_, j) => samples_arr[j][i]).reduce((a, b) => a + b)));
}
