...
const freq_ratio = freq_ratio_sqrt.map(x => x * x / freq_ratio_sqrt[0] / freq_ratio_sqrt[0]);
...
gen_sound({ outPath: "sawtooth_equivalent.wav", samples: gen_timbre_samples({ fundamentalFrequency: PITCH_STANDARD, lengthInSecond: 1, powerSpectrum: (k) => 1 / k }) });
...
function genChord(semitones_arr, outPath) {
    ...
    const samples_arr = semitones_arr.map((semitones) => gen_timbre_samples({
    ...
    gen_sound({
        outPath,
        samples: Array.from({ length: lengthInSecond * SAMPLING_FREQUENCY }, (v, i) => semitones_arr.map((_, j) => samples_arr[j][i]).reduce((a, b) => a + b))
    });
}
